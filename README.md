...
| Demo / kiểm thử  | [Minh họa luồng xử lý và bằng chứng vận hành](#minh-họa-luồng-xử-lý-và-bằng-chứng-vận-hành), [Kịch bản demo đề xuất](#kịch-bản-demo-đề-xuất), [Kiểm thử tải với JMeter](#kiểm-thử-tải-với-jmeter) |
...
| `order-service` | `8086` | Tạo đơn, điều phối checkout saga, outbox, bằng chứng quản trị |
...
| `order-service` | `lsf-kafka-starter`, `lsf-eventing-starter`, `lsf-outbox-mysql-starter`, `lsf-outbox-admin-starter`, `lsf-kafka-admin-starter`, `lsf-saga-starter`, `lsf-observability-starter` | Saga checkout, reliable status event publishing, bằng chứng quản trị |
...
## Minh họa luồng xử lý và bằng chứng vận hành

Các hình dưới đây giúp người đọc nắm nhanh cách hệ thống vận hành mà chưa cần chạy source. LSF không đặt ảnh kịch bản nghiệp vụ ở README framework; toàn bộ bằng chứng khi hệ thống đang chạy được gom ở consumer này vì đây là nơi thể hiện framework khi áp dụng vào một hệ microservices cụ thể.
...
| Workflow/Saga console | Theo dõi order, saga, timeout, reservation, outbox và health từ một màn hình bằng chứng | `order-service`, LSF saga/admin starters |
...
Framework Console là màn hình bằng chứng khi demo. Từ một order number, người dùng có thể kiểm tra workflow, saga/timeout, reservation, outbox/Kafka admin và health/metrics mà không phải đọc log thủ công.
...
| 4. Bằng chứng vận hành trực quan, không cần đọc log | Mở Framework Console, Grafana, Prometheus và Zipkin; kiểm tra saga snapshot, outbox rows, Kafka/DLQ, health/metrics | Người demo có thể giải thích trạng thái hoạt động của LSF thông qua giao diện và dashboard, thay vì phải mở log của từng service |
...
| Bằng chứng quản trị | `GET /api/system/outbox/**`, `GET /api/system/kafka/**`, `GET /api/system/saga/**` qua gateway |
...
> Các tài khoản demo trên chỉ dùng cho môi trường demo/local và phục vụ bảo vệ luận văn. Không dùng lại cho production hoặc môi trường public.
...
| Bằng chứng quản trị | Có outbox, Kafka DLQ và saga snapshot qua gateway |
| Production hardening | Chưa phải mục tiêu chính của repo |

## Lưu ý quan trọng

- Không dùng tài khoản demo cho môi trường thật.
...
- `lsf-saga-starter` trong dự án này có bằng chứng vận hành hữu ích, nhưng vẫn nên mô tả là partial support thay vì workflow engine tổng quát sẵn sàng cho môi trường production.
